<div class="min-h-[80vh] bg-slate-50">
  ...
  <main class="mx-auto max-w-6xl px-4 sm:px-6 py-4 space-y-4 sm:space-y-6">
      ...
      <div class="px-4 sm:px-6 py-5">
        ...
        @if(!$editando)
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            @php
              $dni = $alumno->dni ?? null;
              $fecha = $alumno->fecha_nacimiento ?? null;
            @endphp
            ...
            <div class="space-y-1 sm:col-span-2">
              ...
              <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-800">
                @php $sp = $alumno->socialProfiles->first(); @endphp
    ...
    <!-- NUEVA SECCIÓN: TUTORÍAS -->
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div class="px-4 sm:px-5 py-3 border-b border-slate-100 flex items-center justify-between">
        <h2 class="text-slate-800 font-semibold">Tutorías disponibles</h2>
        <span class="text-sm text-slate-500">Elige una para seguimiento</span>
      </div>

      <div class="px-4 sm:px-6 py-5">
        <form wire:submit.prevent="elegirTutoria" class="space-y-4">
          <div class="grid sm:grid-cols-2 gap-3">
            @foreach($tutorias as $t)
              <label class="inline-flex items-center gap-2 rounded-xl border px-3 py-2">
                <input type="radio" wire:model="selectedTutoria" value="{{ $t }}" class="form-radio">
          ...
          <div class="flex items-center gap-3 pt-3">
            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white hover:bg-blue-700">
              Anotarme en esta tutoría
            </button>

            @if($selectedTutoria)
              <div class="text-sm text-slate-600">Seleccionado: <span class="font-medium text-slate-800">{{ $selectedTutoria }}</span></div>
            @endif
          </div>
        </form>
      </div>
                    ...
                    @if($sp->github)
                      <a href="{{ $sp->github }}" target="_blank" rel="noopener" title="GitHub" class="text-gray-800 hover:text-black">
                        <svg class="w-6 h-6 inline" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M12 .5a12 12 0 00-3.79 23.4c.6.11.82-.26.82-.58v-2.02c-3.34.73-4.04-1.61-4.04-1.61-.55-1.4-1.35-1.77-1.35-1.77-1.1-.75.08-.74.08-.74 1.22.09 1.87 1.26 1.87 1.26 1.08 1.86 2.83 1.32 3.52 1.01.11-.8.42-1.32.76-1.62-2.66-.3-5.46-1.33-5.46-5.9 0-1.3.47-2.36 1.24-3.19-.13-.31-.54-1.55.12-3.23 0 0 1.01-.32 3.3 1.22a11.5 11.5 0 016 0c2.29-1.54 3.0-1.22 3.3-1.22.66 1.68.25 2.92.12 3.23.77.83 1.24 1.89 1.24 3.19 0 4.58-2.81 5.6-5.49 5.9.43.37.81 1.1.81 2.22v3.29c0 .32.22.69.83.58A12 12 0 0012 .5z" />
                        </svg>
                      </a>
                    @endif
                    @if($sp->gitlab)
